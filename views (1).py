...
def testcaseInputChange(request, testcaseID):
    ...
    # TODO: set to_be_evaluated flag true
    # TODO: log the changes
        files = [[testcase.std_in_file_name, testcase.input_files.file.name],[testcase.std_out_file_name, testcase.output_files.file.name]]
        str_input = request.POST['new_input']
        str_output = request.POST['new_output']
        for index in range(2):
            name = files[index][1]
            print(name)
            readthis = files[index][0]
            print(readthis)
            if index == 0:
                string = str_input
            else:
                string = str_output
            basedir = '/'.join(name.split('/')[:-1]) + "/"
            if tarfile.is_tarfile(name):
                tarfile_name = name[0:-7]
                temp_tarfile_name = tarfile_name + "_temp.tar.gz"
                os.rename(name, temp_tarfile_name)
                read_dir = readthis.split("/")[0]
                tarread = tarfile.open(temp_tarfile_name, "r:gz")

                paths = [path for path in tarread.getnames() if  path.split('/')[-1] == readthis]
                readthis = paths[0]
                read_dir = readthis.split("/")[0]
                tarwrite = tarfile.open(name, "w:gz")
                os.chmod(name, 0o777)
                for member in tarread.getmembers():
                    if member.name.split('/')[0] == read_dir and member.name!= readthis:
                        tarread.extract(member)

                for member in tarread.getmembers():
                    if member.name.split('/')[0] != read_dir:
                        tarread.extract(member)

                for member in tarread.getmembers():
                    if len(member.name.split('/')) == 1 and member.name!= read_dir:
                        tarwrite.add(member.name, arcname = member.name)
                        if "/" in member.name:
                ...
                f2 = open(readthis, 'w+')
                os.chmod(readthis, 0o777)
                f2.write(string)
                f2.close()
                tarwrite.add(read_dir, arcname= read_dir)
                ...
                zipfile_name = name[0:-4]
                temp_zipfile_name = zipfile_name + "_temp.zip"
                os.rename(name, temp_zipfile_name)
                read_dir = readthis.split("/")[0]
                zipread = ZipFile(temp_zipfile_name, "r")
                namelist = [member.filename for member in zipread.infolist()]
                paths = [path for path in namelist if path.split('/')[-1] == readthis]
                readthis = paths[0]
                read_dir = readthis.split("/")[0]
                zipwrite = ZipFile(name, "w")
                os.chmod(name, 0o777)
                for member in zipread.infolist():
                    if member.filename.split('/')[0] == read_dir and member.filename != readthis:
                        zipread.extract(member)
                        zipwrite.write(member.filename)

                for member in zipread.infolist():
                    if member.filename.split('/')[0] != read_dir:
                        zipread.extract(member)
                        zipwrite.write(member.filename)

                for member in zipread.infolist():
                    if len(member.filename.split('/')) == 1 and member.filename != read_dir:
                        shutil.rmtree(member.filename)
                ...
                zipwrite.write(readthis)
                os.remove(temp_zipfile_name)
                if "/" in readthis:
                    shutil.rmtree(read_dir)
                else:
                    os.remove(read_dir)
            else:
                f2 = open(basedir + readthis, 'w+')
                f2.write(string)
                f2.close()

        mark_submissions_false(assignment.id)
        return HttpResponse("Input and Output files have been updated")
    return HttpResponseForbidden("Forbidden 403")
